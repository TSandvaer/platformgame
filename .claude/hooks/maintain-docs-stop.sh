...
#
# Pure Q&A / read-only turns exit 0 silently — no "blocking error" banner, no
# skill invocation. This mirrors the skill's own Step 1 early-exit filter one
# level earlier so the UI does not surface a banner for turns the skill would
# early-exit on anyway.
#
