class PropCollisions {
    ...
    checkDamageCollisions(player, props, propTypes) {
        // Check collision with props that have damage (exclude destroyed/destroying/invisible props)
        props.filter(prop => prop.damagePerSecond > 0 && !prop.isDestroyed && !prop.isDestroying && prop.isVisible !== false).forEach(prop => {
            if (this.checkCollision(player, this.getPropBounds(prop, propTypes))) {
